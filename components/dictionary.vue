      ...
      <view class="word_name">
			...
			<view class="definition">
				<view class="dif-item" v-for="(item, index) in classLst" :key="index">
					<view class="class-item" v-if="meaning[item.name]">
						<view class="single-item" v-for="(def, index) in meaning[item.name]" :key="index">
							<view class="short">
								{{item.short}}
							</view>
							<view class="meaning">
		...
		<view class="no-data" v-if="definition == null" :key="numberkey">
		  <u-image
		    ...
		    bg-color="#ffffff"
		  ></u-image>
		  <text class="text">Sorry pal, we couldn't find definitions for the word you were looking for. You can try the search again at later time or head to the web instead.</text>
		</view>
		<u-toast ref="uToast" />
