					...
					<u-row>
						<u-col :span="10" :offset="1" stop>
							<u-search v-model="value" @focus="getFocus" @blur="loseFocus" @change="change"
								:action-style="{width:'100rpx',color:'#007aff'}" @custom="custom" @search="search"
								:shape="'square'" :clearabled="true" :show-action="true" action-text="Search"
			...
			<u-row v-if="onFocus" class="word-selection" style="align-items: flex-start;">
				<u-col :span="10" :offset="1" stop>
					<u-swipe-action @open="open(item)" :index="index" v-for="(item, index) in list1" :key="item.id"
						...
						:disabled="item.show" @content-click="contentClick">
						<view class="item u-border-bottom">
							...
							<u-icon @click="cancelCollected(item)" :name="!item.collected ?'star':'star-fill'" size="40"
								:color="!item.collected? '#909399':'#FF9900'"></u-icon>
