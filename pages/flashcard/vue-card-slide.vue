	...
	<div class="wrap">
		...
		<div class="bg" id='bg'>
			<v-touch v-if="flashWords.length>num" v-on:swipeleft="onSwipeLeft" v-on:swiperight="onSwipeRight">
				...
				<!-- <div class='judge'>
          <div class="circle cirMargin" @click="success()"><span>Forget</span></div>
          <div class="circle" @click="error()"><span>Remember</span></div>
        </div> -->
			</v-touch>
			<view v-else>
	...
	export default {
		...
		methods: {
			...
			success(numb) {
				...
				// 第二个动效
				if (this.num % 3 == 2) {
					this.word1 = this.flashWords[this.num+1]
					...
					setTimeout(() => {
						...
						for (var i = 0; i < 200; i++) {
							...
							if(this.flashWords.length-this.num!==1){
								this.show2 = true;
							}
							this.flag = 'on'
								// this.previousNum++;
				...
				if (this.num % 3 == 0) {
					this.word2 = this.flashWords[this.num+1];
					if(this.num+1>=this.flashWords.length){
						this.word2 = this.flashWords[0]
					}
					// this.previousTurn++;
					this.show3 = false;
					// 第二个变大上移
